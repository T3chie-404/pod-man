...
function extractFlagsFromText(text) {
  if (!text || typeof text !== 'string') {
    return { atlasCluster: null, prpcMode: null, operatorRevenue: null };
  }
  ...
  const clusterMatch = text.match(/--atlas-cluster(?:=|\s+)(trynet|devnet|mainnet-alpha)\b/i)
    ...
    || text.match(/--(trynet|devnet|mainnet-alpha)\b/i);
  ...
  const operatorRevenueMatch = text.match(/--operator-revenue(?:=|\s+)(\d+)\b/i)
    || text.match(/--op-rev(?:=|\s+)(\d+)\b/i)
    || text.match(/operator[_-]revenue(?:\s*[:=]\s*|\s+)(\d+)\b/i);
  ...
  return {
    ...
    operatorRevenue: operatorRevenueMatch ? operatorRevenueMatch[1] : null
  };
}
...
function extractFlagsFromObject(value) {
  ...
  const keysToTry = [
    ...
    'operator_revenue',
    'operatorRevenueBps',
    'operator_revenue_bps'
  ];

  let atlasCluster = null;
  let prpcMode = null;
  let operatorRevenue = null;
  ...
  for (const key of keysToTry) {
    ...
    if (!operatorRevenue && (typeof current === 'string' || typeof current === 'number')) {
      const normalized = String(current).trim();
      if (/^\d+$/.test(normalized)) {
        operatorRevenue = normalized;
      }
    }
  }

  for (const nested of Object.values(value)) {
    if (atlasCluster && prpcMode && operatorRevenue) {
      break;
    }
    if (nested && typeof nested === 'object') {
      ...
      operatorRevenue = operatorRevenue || child.operatorRevenue;
    }
  }

  return { atlasCluster, prpcMode, operatorRevenue };
}
...
async function detectNodeConfiguration() {
  ...
  for (const service of ['xandminer', 'xandminerd', 'pod']) {
    ...
    if (extracted.atlasCluster || extracted.prpcMode || extracted.operatorRevenue) {
      atlasCluster = atlasCluster || extracted.atlasCluster;
      prpcMode = prpcMode || extracted.prpcMode;
      operatorRevenue = operatorRevenue || extracted.operatorRevenue;
  ...
  if (!operatorRevenue) {
    warnings.push('Unable to auto-detect operator revenue from known service/config locations');
  }

  return {
    atlasCluster: atlasCluster || null,
    prpcMode: prpcMode || null,
    operatorRevenue: operatorRevenue || null,
...
async function getStorageState() {
  const storageLinkPath = '/run/xandeum-pod';
  const standardStorageTarget = '/xandeum-pages';
  let storageFileExists = false;
  let storageSymlinkExists = false;
  let storageSymlinkIsSymbolicLink = false;
  let storageSymlinkOk = false;
  let storageSymlinkTarget = null;
  let storageTargetResolved = null;
  let storageTargetExists = false;
  let storageUsesCustomTarget = false;

  try {
    await fsp.stat(standardStorageTarget);
  ...
  try {
    const linkStat = await fsp.lstat(storageLinkPath);
    storageSymlinkExists = true;
    if (linkStat.isSymbolicLink()) {
      storageSymlinkIsSymbolicLink = true;
      const target = await fsp.readlink(storageLinkPath);
      storageSymlinkTarget = target;
      storageTargetResolved = path.resolve(path.dirname(storageLinkPath), target);
      storageUsesCustomTarget = storageTargetResolved !== standardStorageTarget;
      try {
        await fsp.stat(storageTargetResolved);
        storageTargetExists = true;
      } catch (error) {
        storageTargetExists = false;
      }
      storageSymlinkOk = storageTargetExists;
    }
  } catch (error) {
    storageSymlinkExists = false;
    storageSymlinkIsSymbolicLink = false;
    storageSymlinkOk = false;
  }

  return {
    storageReady: storageSymlinkExists && storageSymlinkIsSymbolicLink && storageTargetExists,
    storageFileExists,
    storageSymlinkExists,
    storageSymlinkIsSymbolicLink,
    storageSymlinkOk,
    storageSymlinkTarget,
    storageTargetResolved,
    storageTargetExists,
    storageUsesCustomTarget
  };
}
...
function buildInstallerCommand(installerPath, atlasCluster, prpcMode, operatorRevenue) {
  return `bash ${installerPath} --non-interactive --update --prpc-mode ${prpcMode} --atlas-cluster ${atlasCluster} --operator-revenue ${operatorRevenue}`;
}
...
async function inspectNode() {
  ...
  return {
    ...
    operatorRevenue: config.operatorRevenue,
    ...
    storageSymlinkIsSymbolicLink: storage.storageSymlinkIsSymbolicLink,
    storageSymlinkOk: storage.storageSymlinkOk,
    storageSymlinkTarget: storage.storageSymlinkTarget,
    storageTargetResolved: storage.storageTargetResolved,
    storageTargetExists: storage.storageTargetExists,
    storageUsesCustomTarget: storage.storageUsesCustomTarget,
