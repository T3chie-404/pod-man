...
function isPackageLockPath(value) {
  ...
  return posixName === 'package-lock.json' || winName === 'package-lock.json';
}
...
async function inspectPodManRepo(targetRef = DEFAULT_TARGET_REF) {
  ...
  try {
    ...
    if (
      ...
      inspection.warnings.push(`Ignoring harmless repo changes: ${summarizeDirtyEntries(inspection.dirtyEntries).join(', ')}`);
    }
  } catch (error) {
