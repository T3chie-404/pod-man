...
function isIgnorableDirtyEntry(entry) {
  return entry?.path === 'package-lock.json';
}
...
function validateCentralConfig(centralManagement = {}) {
  ...
  if (enabled) {
    ...
    if (!centralManagement.centralSshUser) {
      missingFields.push('centralManagement.centralSshUser');
    }
  ...
  return {
    ...
    preserved: {
      ...
      centralSshUser: centralManagement.centralSshUser || '',
...
async function inspectPodManRepo(targetRef = DEFAULT_TARGET_REF) {
  ...
  try {
    ...
    if (inspection.dirty) {
      inspection.blockingReasons.push('pod-man repo has modified tracked files or non-ignored untracked files');
    }
