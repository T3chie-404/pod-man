...
async function readGitStatusPorcelain() {
  const { stdout } = await execFileAsync('git', ['-C', PODMAN_REPO_DIR, 'status', '--porcelain=1', '--untracked-files=all'], {
    maxBuffer: 1024 * 1024
  });
...
async function clearIgnorableDirtyEntries() {
  ...
  await execFileAsync('git', ['-C', PODMAN_REPO_DIR, 'restore', '--staged', '--worktree', '--source=HEAD', '--', ...uniquePaths], {
    maxBuffer: 1024 * 1024
  });
