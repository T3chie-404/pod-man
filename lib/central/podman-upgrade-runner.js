...
async function main() {
  ...
  await log(`Using pod-man repo at ${repoDir}`);
  ...
  try {
    await runCommand('git', ['-C', repoDir, 'fetch', 'origin', '--prune']);
  } catch (error) {
  ...
  try {
    ...
    await runCommand('git', ['-C', repoDir, 'checkout', '-B', targetRef, `origin/${targetRef}`]);
    await runCommand('git', ['-C', repoDir, 'reset', '--hard', `origin/${targetRef}`]);
  } catch (error) {
  ...
  try {
    await runCommand('npm', ['install', '--production'], { cwd: repoDir });
  } catch (error) {
  ...
  try {
    const serviceUpdate = await ensurePodManagerService(repoDir);
    await persist({
      result: {
