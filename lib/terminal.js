...
class TerminalManager {
  ...
  createSession(sessionId, cols = 80, rows = 24, userRole = 'admin') {
    try {
      ...
      if (userRole === 'demo') {
        const { uid, gid } = this.getUserIds('demo-user');
        // Demo user: spawn as demo-user with restricted environment
        ptyProcess = pty.spawn('/bin/bash', ['-l'], {
          name: "xterm-color",
          cols: cols,
          rows: rows,
          cwd: "/home/demo-user",
          ...
          gid,
          env: {
            PATH: "/home/demo-user/bin",
            HOME: "/home/demo-user",
            USER: "demo-user",
            SHELL: "/usr/bin/rbash",
            BASH_ENV: "/home/demo-user/.bashrc"
          }
        });
        this.logActivity(sessionId, "SESSION_CREATED", `Demo shell as demo-user (restricted)`);
      } else if (userRole === 'standard') {
        const { uid, gid } = this.getUserIds('standard-user');
        ptyProcess = pty.spawn('/bin/bash', ['-l'], {
          name: "xterm-color",
          cols: cols,
          rows: rows,
          cwd: "/home/standard-user",
          uid,
          gid,
          env: {
            PATH: "/home/standard-user/bin",
            HOME: "/home/standard-user",
            USER: "standard-user",
            SHELL: "/usr/bin/rbash",
            BASH_ENV: "/home/standard-user/.bashrc"
          }
        });
        this.logActivity(sessionId, "SESSION_CREATED", "Standard shell as standard-user (restricted)");
      } else {
  ...
  getUserIds(username) {
    const uid = Number(execSync(`id -u ${username}`, { encoding: "utf8" }).trim());
    const gid = Number(execSync(`id -g ${username}`, { encoding: "utf8" }).trim());
    return { uid, gid };
  }
}
