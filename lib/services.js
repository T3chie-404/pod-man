...
class ServiceManager {
  ...
  static async getStatus(serviceName) {
    ...
    try {
      ...
      // A non-zero exit from `systemctl status` is normal for inactive/failed units.
      // Return the status payload rather than treating the status lookup itself as a failure.
      const output = error.stdout || error.stderr || error.message;
      const lowerOutput = String(output).toLowerCase();
      const isFailed = lowerOutput.includes("failed");
      return {
        success: true,
        service: serviceName,
        output,
        running: false,
        state: isFailed ? "failed" : "stopped",
  ...
  static async controlService(serviceName, action) {
    ...
    // Prevent self-stop (would lock out the UI)
    if (serviceName === 'pod-manager' && action === 'stop') {
      return {
        success: false,
        service: serviceName,
        action: action,
        error: 'Cannot stop monitor service from within itself. Use: sudo systemctl stop pod-manager.service'
      };
    }

    // Allow a delayed self-restart so the request can return before the UI disconnects.
    if (serviceName === 'pod-manager' && action === 'restart') {
      try {
        await execPromise(`nohup bash -lc 'sleep 1; systemctl restart pod-manager.service' >/dev/null 2>&1 &`);
        return {
          success: true,
          service: serviceName,
          action,
          output: 'pod-manager restart scheduled',
          status: {
            success: true,
            service: serviceName,
            running: true,
            state: 'restarting',
            output: 'Restart scheduled; the web UI may disconnect briefly.'
          }
        };
      } catch (error) {
        return {
